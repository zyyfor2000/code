...
#  精确方法求解
    ...
    for i in range(n_replication):
        v = sigma2_hat_1[i] * np.array(
            [[(4 * n + 2) / (n * (n - 1)), -6 / (n * (n - 1))], [-6 / (n * (n - 1)), 12 / (n * (n ** 2 - 1))]])
        var_mu = np.array([L, 0.5 * (L ** 2 + 2 * n * L + L)]) @ v @ np.array(
            [[L], [0.5 * (L ** 2 + 2 * n * L + L)]])
    ...
    print(S_exact, TC_exact )
    # 经典方法求解
    S_classic = L * (alpha_hat) + 0.5 * (L ** 2 + 2 * n * L + L) * beta_hat + sqrt(L) * (-norm.isf(q=b / (b + h))) * sqrt(sigma2_hat)
    z_classic = 0
...
for n in [5, 10, 20, 100]:
    ...
    for i in range(n_replication):
        ...
        z_real.append(norm.rvs(loc=L * mu_hat_1, scale=sqrt(L * sigma2_hat_1[i]), size=1))
