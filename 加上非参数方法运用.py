...
for n in [5,10,20,100]:
    ...
    for r in range(r_all):
        ...
        #print("n={},mu_hat={}, sigma2_hat={} ".format(n,mu_hat,sigma2_hat))
    # 精确方法求解
        z_exact = []
    ...
    # 渐进方法求解
        ...
        S_22 = np.mean(S)*L
        z_22 = S_22 - x_l_all
        TC_22 = (sum(h * z_21[z_22 > 0]) -
                 sum(b * z_21[z_22 < 0])) / n_replication
        tc_np[r, 2] = TC_22
       # 3. SVP1-3
        ...
        * data_ordered[n - 3] + binom.pmf(n, n, p) / 2 * data_ordered[n - 2] + (2 * binom.pmf(n, n, p)  \
        ...
        tc_np[r,5] = TC_33
    print(np.mean(tc_all, axis=0))
    print(np.mean(tc_np,axis=0))
end = time.time()
print((end-begin)/60)
